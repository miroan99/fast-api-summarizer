...
limiter = Limiter(key_func=get_remote_address, default_limits=["30/minute"])  # default: 30 req/min per IP
...
class SummarizeRequest(BaseModel):
    text: constr(min_length=1, max_length=10_000) = Field(..., description="Input text to summarize (max 10k chars)")
    max_words: conint(ge=10, le=300) = Field(120, description="Max words in the summary (10-300)")
    language: Optional[str] = Field(None, description="Output language code, e.g. 'da' or 'en'. If omitted, auto-detect.")
    tone: Optional[str] = Field(None, description="Optional tone: 'neutral', 'formal', 'casual', etc.")
...
@app.get("/health")
@limiter.limit("10/second")  # very generous for health checks
async def health():
    return {"status": "ok"}


@app.post("/summarize", response_model=SummarizeResponse, dependencies=[Depends(require_api_key)])
@limiter.limit("10/minute")  # tighten if needed
async def summarize(payload: SummarizeRequest):
    ...
    if not OPENAI_API_KEY and not OPENAI_BASE_URL:
        ...
        raise HTTPException(status_code=400, detail="Missing OPENAI_API_KEY. Set it in .env. If using LM Studio, set OPENAI_BASE_URL and a dummy key.")

    # Build prompt with minimal metadata; avoid leaking secrets in errors
    lang_hint = f" Danish (da)" if (payload.language or "").lower().startswith("da") else (" English (en)" if payload.language else "")
